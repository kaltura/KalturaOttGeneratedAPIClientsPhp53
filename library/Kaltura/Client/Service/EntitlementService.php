...
class EntitlementService extends \Kaltura\Client\ServiceBase
{
	 ...
	 * 
	 * @return \Kaltura\Client\Type\EntitlementRenewal
	 */
	function getNextRenewal($id)
	{
		...
		$resultXmlObject = new \SimpleXMLElement($resultXml);
		$this->client->checkIfError($resultXmlObject->result);
		$resultObject = \Kaltura\Client\ParseUtils::unmarshalObject($resultXmlObject->result, "KalturaEntitlementRenewal");
		$this->client->validateObjectType($resultObject, "\\Kaltura\\Client\\Type\\EntitlementRenewal");
		return $resultObject;
	}
